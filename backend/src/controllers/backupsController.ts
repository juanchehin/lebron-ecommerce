...
// ==================================================
public async alta_backup(req: Request, res: Response) {

  let dateTime = new Date();
  let nameBackup = `lebron-${dateTime.toISOString().slice(0, 10)}.sql`;
  var dir: any;
  ...
  try{
   ...
   pool.query(`call bsp_alta_backup('${nameBackup}')`, function(err: any, result: any, fields: any){
    ...
    dir = `./var/lebron/backups/${result[0][0].Id}`;
      ...
      pool.query(`call bsp_dame_config_db()`, function(err: any, result: any){
        if(err){
          logger.error("Error bsp_dame_config_db - alta_backup - backupController " + err + " " + result);
        ...
        fs.appendFile(`${dir}/MySQLCurrentSettings-${dateTime.toISOString().slice(0, 10)}.json`, JSON.stringify(result), function (err: any) {
          if(err){
            logger.error("Error en fs.appendFile - backupController " + err + " " + result);
  ...
  */ 
  function uploadFile(auth: any) {
    ...
    for (const file of fs.readdirSync(`./backup/${id}`)) {
      ...
      const media = {
        mimeType: 'text/sql',
        body: fs.createReadStream(`./var/lebron/backups/${id}/${file}`)
      };
      drive.files.create({
        ...
        if (err) {
          logger.error("Error en uploadFile - backupController " + err);
          // Handle error
          console.error(err);
          res.json({ Mensaje: 'Error' });
          return;
  ...
  pool.query(`call bsp_backup_sinc('${id}')`, function(err: any, result: any, fields: any){
    if(err){
      logger.error("Error en bsp_backup_sinc - backupController " + err + " " + result);
